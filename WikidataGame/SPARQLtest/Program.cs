...
namespace SPARQLtest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            quests.Add("What is the capital of {0}?", "SELECT ?answer ?question WHERE { ?item wdt:P31 wd:Q5119. ?item wdt:P1376 ?land. ?land wdt:P31 wd:Q6256. OPTIONAL { ?item rdfs:label ?answer; filter(lang(?answer) = 'en') ?land rdfs:label ?question; filter(lang(?question) = 'en').} }  ORDER BY MD5(CONCAT(STR(?item), STR(NOW()))) LIMIT 4");
